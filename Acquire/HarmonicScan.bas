...
Const Samp As Double = 5000
Const SampFreq As Double = 781250
Const GenAmp As Double = 0.05
...
' -------------------------------------------------------------------------------
	...
	For dblFreqScan = hsFreqStart To hsFreqEnd Step hsFreqStep
		...
		Debug.Print dblFreqScan
		'Dim Acq As Acquisition

		'Acq = Application.Acquisition

        Acquisition.Mode = PTCAcqMode.ptcAcqModeTime

		Dim ActiveProp As AcquisitionProperties
		ActiveProp = Acquisition.ActiveProperties
		...
		Call UpdateFreq(dblFreqScan)


        Acquisition.ScanFileName = scanFileNames(i)
...
Private Sub UpdateFreq(UpFreq%)
	...
	Dim Sine As New WaveformSine
	Sine.Frequency = UpFreq	' Hz
	GenProps.Waveform = Sine
	GenProps.Amplitude = GenAmp
	GenProps.SteadyStateTime = GenSteadyState
...
' -------------------------------------------------------------------------------
	...
	Case 1 ' Dialog box initialization.
		DlgText   "fStart"    , "22100"
		DlgText   "fEnd"    , "23000"
		DlgText   "fStep"    , "100"
		DlgText	  "tAvgs"	 ,	"3"
		DlgText		"sFile"	 , 	"D:/Sai/0909 Transistor/SS.svd"
